...
import apis.icco_api_timezone as icco_api_timezone
import table_paginate
import apis.api_methods as reApi
app = Flask(__name__)
CORS(app)
...
class GetGraphData(MethodView):
    ...
    def post(self):
        data_dict = request.get_json()
        if 1:#try:
            obj = table_paginate.table_paginate('142.93.208.71')
            def_dict = obj.getAllGraphData(data_dict)
        return jsonify(def_dict)

app.add_url_rule("/cqICCOpApi/GetGraphData", view_func = GetGraphData.as_view("GetGraphData"))

class GetGraphDataNew(MethodView):
    def get(self):
        """ Responds to GET requests """
        return "Responding to a GET request = "
...
app.add_url_rule("/cqICCOpApi/GetGraphDataNew", view_func = GetGraphDataNew.as_view("GetGraphDataNew"))
...
class GetDomainsByCustomer(MethodView):
    ...
    def post(self):
        if 1:#try
            ...
            print(customer_name)
            def_dict=api41.get_domains_by_customer(customer_id,customer_name)
        return jsonify(def_dict)
...
class UpdateDomainSubscription(MethodView):
    ...
    def post(self):
        if 1:#try
            ...
            tdata = data_dict.get("tdata")
            print(uid,user,tdata)
            def_dict=api41.update_domain_subscription(uid,user,tdata)
        return jsonify(def_dict)
...
class GetTimeZoneList(MethodView):
    def get(self):
        """ Responds to GET requests """
        #return "Responding to a GET request"
        return
    ...
    def post(self):
        if 1:#try
            data_dict = request.get_json()
            def_dict=icco_api_timezone.get_timezone(data_dict)
        return jsonify(def_dict)

app.add_url_rule("/cqICCOpApi/GetTimeZoneList", view_func =  GetTimeZoneList.as_view("GetTimeZoneList"))
...
class ResetPassword(MethodView):
    ...
    def post(self):
        if 1:#try
            data_dict = request.get_json()
            def_dict=reApi.reset_password_new(data_dict)
        return jsonify(def_dict)

app.add_url_rule("/cqICCOpApi/ResetPassword", view_func =  ResetPassword.as_view("ResetPassword"))
