...
def get_clients(dbstring):
    conn, cur = get_connection(dbstring)
    sql = "SELECT customer_name FROM customer_info"
    cur.execute(sql)
    res = cur.fetchall()
    data = [r[0]for r in res]
    cur.close()
...
def get_domains_by_customer(dbstring, customer_id):
    conn, cur = get_connection(dbstring)
    sql = "SELECT subscription_id,customer_domain FROM customer_subscription WHERE customer_id = '%s'"
    cur.execute(sql%customer_id)
    res = cur.fetchall()
    data = [[r[0],r[1]] for r in res]
    cur.close()
...
def insert_into_user_customer_info(dbstring,vals):
    conn, cur = get_connection(dbstring)
    sql = """insert into user_customer_info(user_id,customer_name,user_name,customer_domain,subscription,d_flag,s_flag,access_key) values(%s,"%s","%s","%s","%s",%s,%s,%s) """
...
def get_customer_details_by_user_id(dbstring,uid):
    conn, cur = get_connection(dbstring)
    sql = "SELECT customer_name,customer_domain,subscription,d_flag,s_flag,access_key from user_customer_info where user_id=%s"
    cur.execute(sql%(uid))
    res = cur.fetchall()
    #print(res)
    cur.close()
...
def get_users_permissions(dbstring):
    conn, cur = get_connection(dbstring)
    sql = "SELECT id,full_name,email,permission from users"
    cur.execute(sql)
    res = cur.fetchall()
    cur.close()
    conn.close()
