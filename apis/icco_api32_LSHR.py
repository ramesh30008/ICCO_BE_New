...
def getHostPoolsLoginSummary(data_dict):
    ...
    for sub_id in subscriptions: 
        ...
        temp_uname = mongo_methods.read_session_mgmt(db_str_mongo_old,"session_mgmt", query)
        uname_dict.update(temp_uname)
...
def change_timezone_users(data_dict, tz):
    res_dict = {}
    for k, val in data_dict.items():
        res_dict[k] = []
        for v in val:
            v = list(v)
            date_str = v[0]+ " "+str(v[1])+":"+str(v[3])
            ...
            v[1] = hour
            v[3] = minute
            res_dict[k].append((v[0],v[1], v[2], v[3]))
    return res_dict 
 
def change_timezone_host(data_dict, tz):
    res_dict = {}
    for k, val in data_dict.items():
        res_dict[k] = []
        for v in val:
...
def getHostPoolsHourlyReportWVD(data_dict):
    ...
    for sub_id in subscriptions: 
        ...
        host_session_ids.update(host_session)
    if tz != "UTC":
        user_session_ids = change_timezone_users(user_session_ids, tz)
        host_session_ids = change_timezone_host(host_session_ids, tz)
...
if __name__ == '__main__':
    ...
    "tags": [],
    "date": [
        "2023-4-27T00:00:00.0000000Z",
        "2023-5-4T00:00:00.0000000Z"
    ],
    "cust_id": 6,
    "cust_name": "HGS CES",
    "filter": [],
    "tz_list": "Asia/Kolkata"
}
    data = getHostPoolsHourlyReportWVD(data_dict)
    #with open("eastern.txt",'r') as f:
    #    f.write(str(data))
