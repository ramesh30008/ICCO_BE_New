...
def getFinalDictTable(final_dict, my_ykeys, all_dates, all_ui_dates, flag, subs):
    ...
    for sub in list(res_dict.keys()):
        if 0 in flag and 1 in flag:
            ...
            if all_zero_p:
                res_dict[sub]["data_rows"][1] = ["NA" for i in range(len(all_dates))]
            all_zero_a = all(v == 0 for v in res_dict[sub]["data_rows"][0])
            if all_zero_a:
                res_dict[sub]["data_rows"][0] = ["NA" for i in range(len(all_dates))]
        else:
    ...
    if 0 in flag and 1 in flag:
        if not total_list:
            total_list = ["NA" for i in range(len(all_dates))]
        if not total_p_list:
            total_p_list = ["NA" for i in range(len(all_dates))]
