...
def customerPageData(data_dict={}):
    uid = data_dict.get("uid", 0)
    c_list = dbApi.get_companies(db_str_mysql, uid)
    cust_sub_ids = dbApi.get_sub_ids_new(db_str_mysql, c_list)
    final_list = []
    count = 0
    ...
    return {"tdata": [day1, day7, day30], "tykeys": list(cust_sub_ids.keys()), "txkeys":xkeys}
