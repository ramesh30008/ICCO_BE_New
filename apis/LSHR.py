...
def getHostPoolsLoginSummary(data_dict):
    ...
    print(len(user_int_sess_ids), len(user_int_sess_ids_new))
    ...
    for k, v in date_wise_dict.items():
        unames = [uname_dict.get(vv) for vv in v['User Sessions']]
        data.append([k, len(v['User Sessions']), ",".join(unames), len(unames), len(set(unames))])
        #print(k,len(unames), len(set(unames)), len( v['User Sessions']))
    print(len(data[0]), data[0])
    df = pd.DataFrame(data,  columns = ["date", "User Session count", "User Names", "ss", 'ss'])
    print(df)
    df.to_csv("/var/www/html/user_session.csv")
...
if __name__ == '__main__':
    ...
    "tags": [],
    "date": [
        "2023-3-01T00:00:00.0000000Z",
        "2023-04-04T00:00:00.0000000Z"
    ],
    "cust_id": 21,
    "cust_name": "ABSLI - AVD",
    "filter": []
