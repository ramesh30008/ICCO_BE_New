...
def filter_data(filter_ip_dict, data_dict, data_key_cat, p_index):
    ...
    for data_key, cost in  data_dict.items():
        ...
        if p_index == 7:
            final_dict[date_str][sub_pkey] = [sub_cost, curr, 'HostPool']
        elif p_index == 4:
...
def getHpCostByDateAndHp(data_dict):
    ...
    hostpool_clicked = data_dict['hostpool']
    ...
    cust_data = mongo_methods.get_hpcost_date(db_str_mongo, collection_name, date_, sid, rg_to_hp_dict, hostpool_clicked)
...
def getCustomerHpFilterData(data_dict):
    cust_name = data_dict["cust_name"]
    res_dict =  mongo_methods.read_customer_filter_data(db_str_mongo, cust_name)
    return {"Subscription": res_dict["sub"], "Region":res_dict["regions"], "Tags":res_dict["tags"], "Hostpools":res_dict['hostpools']}

def customerHpUsageDetails(data_dict = {}):
    ...
    for (sub, month_year) in itertools.product(*[subs, all_months_years]):
        ...
        temp_dict = mongo_methods.get_hpcost_new(db_str_mongo, collection_name, sub, rg_to_hp_dict)
        cust_data.update(temp_dict)
    final_dict, all_ykeys = filter_data(data_dict, cust_data, data_key_cat, 7)
    ...
    return {"txkeys":all_ui_dates, "tykeys":ntykeys, "tdata":final_tdata_arr, "gdata":final_gdata_arr, "gxkeys":all_ui_dates, "gykeys":graph_ykeys,"key": ndata_dict["key"], "total_pages": ndata_dict["total_pages"], "records": ndata_dict["records"] }
...
if __name__ == '__main__':
    data_dict = {
    "cust_id": 16,
    "cust_name": "DR_TranTax",
    "hostpools": [
        "1006_sp_test",
        "23_sp_2"
    ],
    "subscriptions": [
        "8ccf20a5-83d7-45a9-ba84-d3f0a0cf93d0"
    ],
    "regions": [
        "centralindia",
        "eastus"
    ...
    print(customerHpUsageDetails(data_dict))
