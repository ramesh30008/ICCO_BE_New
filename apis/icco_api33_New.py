...
def filter_data(filter_ip_dict, data_dict, data_key_cat, p_index):
    ...
    for data_key, cost in  data_dict.items():
        ...
        if p_index == 6:
            final_dict[date_str][sub_pkey] = [sub_cost, curr, 'Resource groups']
        elif p_index == 4:
...
def getFinalDictTable(final_dict, my_ykeys, all_dates, all_ui_dates, cat, currency):
    data_dict_arr =[]
    for i, date1 in enumerate(all_dates):
        date2 = all_ui_dates[i]
        ykey_dict = final_dict.get(date1, {})
        ...
        total = 0
        for ykey in my_ykeys:
            cost_tup = ykey_dict.get(ykey, [0, currency, cat])
            temp_dict[ykey] = cost_tup[0]
            temp_dict['%s_Curr'%ykey] = cost_tup[1]
            temp_dict['%s_P'%ykey] = cost_tup[2]
            total+=cost_tup[0]
...
def getRgCostByDateAndRg(data_dict):
    ...
    data_key_cat = ['date', "regions", "subscriptions", "rproviders", "resource", "currency" , "resource_group", "hostpools"]
    cust_data = {}
    rg_to_hp_dict  = {}
    for sid in sids:
        sid_new = sid.replace("-","")
        collection_name = "%s_%s_%s"%(sid_new, m, y)
        temp_rg_to_hp_dict = mongo_methods.get_all_hostpool_rgrps(db_str_mongo, sid)
        temp_cust_data = mongo_methods.get_rgcost_date(db_str_mongo, collection_name, date_, sid, rg_to_hp_dict, resource_grp_clicked)
        cust_data.update(temp_cust_data)
        rg_to_hp_dict.update(temp_rg_to_hp_dict)
...
def getCustomerRgFilterData(data_dict):
...
def customerRgUsageDetails(data_dict = {}):
    ...
    for (sub, month_year) in itertools.product(*[subs, all_months_years]):
        ...
        temp_dict = mongo_methods.get_rgcost_new(db_str_mongo, collection_name, sub, rg_to_hp_dict)
        cust_data.update(temp_dict)
    currency = mongo_methods.get_currency(db_str_mongo, collection_name_list)
    ...
    final_tdata_arr = getFinalDictTable(final_dict, all_ykeys, all_dates, all_ui_dates, "Resource groups", currency)
...
if __name__ == '__main__':
    ...
    "subscriptions": [
        "startek_ri_cost",
        "14c7c8cf-8dd1-444d-a18a-caf321dc2011",
        "4648c78f-0f5c-4ee8-9dbd-4feecfb42599",
        "686356e4-ea06-4c06-8357-a96b7b2f4e67"
    ],
    "regions": [
        "us east",
        "westus2",
    ...
    "tags": [],
    "date": [
        "2023-5-1T00:00:00.0000000Z",
        "2023-5-15T00:00:00.0000000Z"
    ],
    "cust_id": 5,
    "cust_name": "Startek",
    "filter": []
}
    print(customerRgUsageDetails(data_dict))
